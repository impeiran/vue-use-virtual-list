...
function useVirtualList(list, options) {
    var _a;
    const container = vueDemi.ref();
    const range = vueDemi.ref({
        start: 0,
        end: 0
    });
    const { itemHeight, itemWidth, overscan, direction } = vueDemi.toRefs(options);
    ...
    const capacity = vueDemi.computed(() => {
        var _a, _b;
    ...
    const virtualList = vueDemi.computed(() => {
        return list.value
    ...
    vueDemi.watch([list.value], calcListRange);
    vueDemi.onMounted(calcListRange);
    return {
        container,
        virtualList,
        style,
